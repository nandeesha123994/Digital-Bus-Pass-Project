-- Comprehensive Database Fix for Bus Pass Management System
-- Run this script to fix all database structure issues

-- 1. Add missing columns to bus_pass_applications table
...
-- 2. Add missing columns to bus_pass_types table
...
-- 5. Create routes table if not exists
...
-- 7. Insert default categories if table is empty
...
-- 8. Insert default routes if table is empty
...
-- 9. Update existing bus_pass_types with amounts if missing
...
-- 10. Generate application_id for existing records without one
...
-- 12. Insert default admin user (password: admin123)
...
ALTER TABLE payments ADD INDEX IF NOT EXISTS idx_application_id (application_id);
